package com.iplusplus.coding.template;
...
public class QuizTemplate implements TemplateLoader {
	...
	public void load() {
		...
		Fixture.of(Protocol.class).addTemplate("valid", new Rule() {{
			add("id", random(Long.class, range(1L, 100L)));
			add("user", random("hez", "aaron"));
			add("quiz", one(Quiz.class, "id"));
			add("startTime", quizTime.getTime());
			add("finishTime", quizTime.getTime().plusMinutes(2));
			add("questionCount", 5);
	...
	QuizTime quizTime = new QuizTime(Clock.fixed(Instant.parse("2018-12-06T10:30:30.00Z"), ZoneId.systemDefault()));
}
